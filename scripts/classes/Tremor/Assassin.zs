...
events.register(function(event4 as EntityLivingDamageEvent){
    ...
    if !event4.damageSource.immediateSource instanceof IPlayer { return; }
...
static lowRangeUUID as string = UUID.randomUUID().asString();
...
events.register(function(event as crafttweaker.event.PotionEffectAddedEvent){
	...
	if event.potionEffect.effectName != "effect.weight" { return; }
	if event.potionEffect.amplifier != 255 { return; }
...
events.register(function(event as crafttweaker.event.PotionEffectRemoveEvent){
    if event.entityLivingBase.world.isRemote() { return; }
    if !event.entityLivingBase instanceof IPlayer return;
    if isNull(event.potionEffect) { return; }
	if event.potionEffect.effectName != "effect.weight" { return; }
	if event.potionEffect.amplifier != 255 { return; }
