...
events.register(function(event4 as EntityLivingDamageEvent){
    ...
    if (isNull(event4.damageSource.trueSource) || !event4.damageSource.trueSource instanceof IPlayer || !event4.damageSource.immediateSource instanceof IPlayer) { return; }
    ...
    if ((hasArmor(helmet, chestplate, leggings, boots) || hasSlimeArmor(helmet, chestplate, leggings, boots)) && wildcards.isWithinLimit(player)) {
...
events.register(function(event5 as crafttweaker.event.EntityLivingDeathEvent){
    ...
    if (isNull(event5.damageSource.trueSource) || !event5.damageSource.trueSource instanceof IPlayer) { return; }
    ...
    if (player.isPotionActive(<potion:quark:blue>) && (hasArmor(helmet, chestplate, leggings, boots) || hasSlimeArmor(helmet, chestplate, leggings, boots)) && wildcards.isWithinLimit(player)) {
...
events.register(function(event as crafttweaker.event.PlayerTickEvent){
    if (event.side == "CLIENT" || event.phase == "START" || event.player.world.time %5 != 0) { return; }
    ...
    if ((hasArmor(helmet, chestplate, leggings, boots) || hasSlimeArmor(helmet, chestplate, leggings, boots)) && wildcards.isWithinLimit(player)) {
        ...
        if (isNull(player.getNBT().ForgeData.sneakTime)) { player.setNBT({sneakTime: 0}); }
        ...
        if (player.isPotionActive(<potion:quark:white>) && !hasGlow(player.currentItem) && !hasGlow(player.offHandHeldItem)) {
        ...
        if player.isInsideOfMaterial(IMaterial.leaves()) {
            if (player.isSneaking && !hasGlow(player.currentItem) && !hasGlow(player.offHandHeldItem)) {
                ...
                if !player.isPotionActive(<potion:minecraft:invisibility>) {
                    ...
                    if sneakTime >= 2 { player.addPotionEffect(<potion:quark:white>.makePotionEffect(70, 0)); }
                }
...
static lowRangeUUID as string = "7d6d3030-b513-430a-ab7c-b6d557cbe3a5";
...
events.register(function(event as crafttweaker.event.PotionEffectAddedEvent){
    ...
    if (event.potionEffect.effectName != "effect.weight" || event.potionEffect.amplifier != 255 || !event.entityLivingBase instanceof IPlayer) return;
    
    var player as IPlayer = event.entityLivingBase;

    player.getAttribute("generic.reachDistance").removeModifier(lowRangeUUID);

    var statReach as AttributeModifier = AttributeModifier.createModifier("statReach", -4, 0, lowRangeUUID);
    val reachDecrease as AttributeInstance = player.getAttribute("generic.reachDistance");

    reachDecrease.applyModifier(statReach);
});

events.register(function(event as crafttweaker.event.PotionEffectRemoveEvent){
    if event.entityLivingBase.world.isRemote() { return; }
    if (isNull(event.potionEffect) || event.potionEffect.effectName != "effect.weight" || event.potionEffect.amplifier != 255 || !event.entityLivingBase instanceof IPlayer) return;
...
events.register(function(event as crafttweaker.event.PotionEffectExpiryEvent){
    if event.entityLivingBase.world.isRemote() { return; }
    if (isNull(event.potionEffect) || event.potionEffect.effectName != "effect.weight" || event.potionEffect.amplifier != 255 || !event.entityLivingBase instanceof IPlayer) return;
