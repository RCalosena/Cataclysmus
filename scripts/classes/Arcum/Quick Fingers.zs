...
events.register(function(event as crafttweaker.event.ArrowLooseEvent){
    ...
    if player.isPotionActive(<potion:potioncraft:usespeed>) {
        player.sendPlaySoundPacket("minecraft:entity.enderdragon.flap", "player", player.position, 2.0, 4.0);
        player.sendPlaySoundPacket("minecraft:entity.enderdragon.flap", "player", player.position, 2.0, 4.0);
        server.commandManager.executeCommandSilent(server,"particle spit " ~ player.x ~ " " ~ (player.y + 1) ~ " " ~ player.z ~ " 0.0 0.0 0.0 0.5 20 force @a");
    }
});
...
events.register(function(event3 as crafttweaker.event.EntityLivingHurtEvent){
    ...
    var player as IPlayer = event3.entity;
    var helmet as IItemStack = player.getItemInSlot(IEntityEquipmentSlot.head());
    var chestplate as IItemStack = player.getItemInSlot(IEntityEquipmentSlot.chest());
    var leggings as IItemStack = player.getItemInSlot(IEntityEquipmentSlot.legs());
    var boots as IItemStack = player.getItemInSlot(IEntityEquipmentSlot.feet());
    ...
    if (!hasArmor(helmet, chestplate, leggings, boots) || !wildcards.isWithinLimit(player)) { return; }
        ...
        if(player.hasGameStage("mounting12")) { player.addGameStage("m12"); }
});
...
if (event2.player.world.time % 20 != 0) { return; }
    ...
    var helmet as IItemStack = players.getItemInSlot(IEntityEquipmentSlot.head());
    var chestplate as IItemStack = players.getItemInSlot(IEntityEquipmentSlot.chest());
    var leggings as IItemStack = players.getItemInSlot(IEntityEquipmentSlot.legs());
    var boots as IItemStack = players.getItemInSlot(IEntityEquipmentSlot.feet());

    if (players.isRiding && hasArmor(helmet, chestplate, leggings, boots) && wildcards.isWithinLimit(players)) { 
    ...
    for MikeHawk in AcidPotion {
        if (mainhand && players.hasGameStage("mounting1") && players.isPotionActive(MikeHawk)) {
            var count = 0;
        ...
        if (players.isPotionActive(usespeed) && players.hasGameStage("mounting1") && players.hasGameStage("m1")) { 
            if players.isPotionActive(MikeHawk) return;
            players.removePotionEffect(usespeed); 
    ...
    for Empty in AirPotion {
        if (mainhand && players.hasGameStage("mounting2") && players.isPotionActive(Empty)) {
            var count1 = 0;
        ...
        if (players.isPotionActive(usespeed) && players.hasGameStage("mounting2") && players.hasGameStage("m2")) { 
            if players.isPotionActive(Empty) return;
    ...
    for nature in lush {
        if (mainhand && players.hasGameStage("mounting3") && listContains([nature], playerBiome.name)) {
            players.addPotionEffect(usespeed.makePotionEffect(inf, 6));
            players.addGameStage("m3");
        }

        if (players.isPotionActive(usespeed) && players.hasGameStage("mounting3") && players.hasGameStage("m3")) { 
            if (listContains([nature], playerBiome.name)) return; 
    ...
    if (players.isPotionActive(usespeed) && players.hasGameStage("mounting4") && players.hasGameStage("m4") && !mobDist(event2, 30.0)) {
        players.removePotionEffect(usespeed);
    ...
    for Dirt in EarthPotion {
        if (mainhand && players.hasGameStage("mounting5") && players.isPotionActive(Dirt)) {
            var count2 = 0;
        ...
        if (players.isPotionActive(usespeed) && players.hasGameStage("mounting5") && players.hasGameStage("m5")) { 
            if players.isPotionActive(Dirt) return;
    ...
    ##Fae Ability##
    if (mainhand && players.hasGameStage("mounting6") && world.isDayTime()) {
        players.addPotionEffect(usespeed.makePotionEffect(inf, 6));
        players.addGameStage("m6");
    }

    if (players.isPotionActive(usespeed) && players.hasGameStage("mounting6") && players.hasGameStage("m6") && !world.isDayTime()) {
        players.removePotionEffect(usespeed);
    ...
    for ColdDrink in FrostPotion {
        ...
        if (mainhand && players.hasGameStage("mounting7")) {    
            ...
            if (players.isPotionActive(usespeed) && players.hasGameStage("B")) { 
                if (players.isPotionActive(ColdDrink)) return;
    ...
    for Soup in LavaPotion{
        ...
        if (mainhand && players.hasGameStage("mounting8")) {
            ...
            if (players.isPotionActive(usespeed) && players.hasGameStage("C")) { 
                if (players.isPotionActive(Soup)) return;
    ...
    for CoCReference in LightningPotion {
        ...
        if (mainhand && players.hasGameStage("mounting9")) {
            ...
            if (players.isPotionActive(usespeed) && players.hasAllGameStages("D", "m9")) { 
                if (players.isPotionActive(CoCReference)) return;
    ...
    ##Nether Ability##
    if (mainhand && players.hasGameStage("mounting10") && !isNull(players.getNBT().ForgeData.kills)) {
    ...
    for V in game.potions {
        if (mainhand && players.hasGameStage("mounting14")) {
            ...
            if players.isPotionActive(V) {
                ...
                for allPotions in game.potions {
                    if (players.isPotionActive(allPotions) && V.badEffect){
                        count6 += 1;
                    }
                }
        ...
        if (players.isPotionActive(usespeed) && players.hasGameStage("mounting14") && players.hasGameStage("m14")) { 
            if players.isPotionActive(V) return;
    ...
    if (mainhand && players.hasGameStage("mounting15")) {
        if (players.isInWater) { 
            players.addPotionEffect(usespeed.makePotionEffect(inf, 6)); players.addGameStage("m15"); 
        } else {
            if (players.hasGameStage("m15")) { players.removePotionEffect(usespeed); players.removeGameStage("m15"); }
        }
    }
    ##Water Ability##
    for gamestages in stages {
        if(!hasArmor(helmet, chestplate, leggings, boots) || !wildcards.isWithinLimit(players)) {
            players.removePotionEffect(usespeed);
            players.removeGameStage(gamestages);
